  ...
  return (
    <div className="risk-factor-row">
      <div className="risk-factor-label">
        <strong>{label}</strong>
        <span>
          {drivers.slice(0, 2).map(translateDriver).join(" · ") ||
            "Sem fator material nesta execução."}
        </span>
      </div>
  ...
  const evidenceRows = useMemo(
    () => [
      ...
      },
      {
        ...
        signal: "Evento ambiental observado",
      ...
      <div className="content">
        ...
        <section className="knowledge-grid" aria-label="Estado das informações">
          <article className="knowledge-card knowledge-known">
            <span>O que sabemos</span>
            <strong>{toPercent(input.evidence_completeness)}%</strong>
          ...
          <article className="knowledge-card knowledge-observed">
            ...
            <p>sinais específicos estão acima do limiar de atenção da demonstração.</p>
          </article>
        ...
        <section className="risk-overview panel">
          ...
          <div className="decision-block">
            ...
            <strong>
              {assessment.human_review_required ? "Revisão humana necessária" : "Sem revisão obrigatória"}
            </strong>
            <p>
          ...
          <div className="dimension-summary">
            ...
            <div>
              <span>Sinal do modelo</span>
              <strong>{mlPrediction ? `${toPercent(mlPrediction.predicted_material_risk_probability)}%` : "—"}</strong>
            </div>
          </div>
        </section>
        ...
        <section className="section-block" id="evidencias">
          <div className="section-heading">
            ...
            <span className="section-note">Sinais sintéticos até a integração de dados públicos entrar nesta branch</span>
          </div>
                ...
                <tbody>
                  {evidenceRows.map((row) => (
                    <tr key={`${row.source}-${row.signal}`}>
                      ...
                      <td><span className={`table-status status-${row.tone}`}>{row.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="section-block" id="fatores">
          <div className="section-heading">
            <div>
              ...
              <p>As dimensões permanecem separadas para evitar que um único número esconda a origem do risco.</p>
            </div>
            <span className="section-note">Escala heurística experimental de 0 a 100</span>
          </div>
        ...
        <section className="section-block" id="metodologia">
          <div className="section-heading">
            <div>
              <h2>Metodologia</h2>
              <p>A complexidade fica disponível para auditoria, mas não compete com a decisão principal.</p>
            </div>
          </div>
            ...
            <div className="methodology-grid">
              <article className="panel method-panel">
                ...
                <div className="method-footnote">
                  ...
                  <span>Completude de evidência excluída das features de ML</span>
                </div>
              </article>
              ...
              <article className="panel review-method-panel">
                <div className="review-method-head">
                  <div>
                    <span className="panel-label">Contraditório independente</span>
                    <h3>Interpretar, contestar, escalar</h3>
                  </div>
                  <button className="button button-secondary" type="button" onClick={runDualModelReview} disabled={aiLoading}>
                ...
                {!aiReview ? (
                  <div className="review-empty">
                    <p>
                      A camada de linguagem fica depois de regras e modelo estatístico. Ela pode explicar,
                      contestar e pedir mais informação, mas não pode reescrever os scores determinísticos.
                    </p>
                  </div>
                ) : (
                  <div className="review-result">
                    <div className="review-row"><span>Análise</span><strong>{aiReview.analyst?.summary ?? "Indisponível"}</strong></div>
                    <div className="review-row">
                      <span>Revisor</span>
                      <strong>{aiReview.reviewer?.verdict ?? "SEM SAÍDA"}</strong>
                      <p>{aiReview.reviewer?.rationale ?? aiReview.degradation_reason}</p>
                    </div>
                    <div className="review-row review-gate-row">
                      <span>Portão de decisão</span>
                      <strong>{aiReview.decision_gate.replaceAll("_", " ")}</strong>
                      <p>{aiReview.disagreement ? "Houve discordância entre modelos. A revisão humana permanece obrigatória." : "Nenhuma discordância material foi registrada nesta execução."}</p>
                    </div>
                    <div className="provider-traces">
                      {aiReview.provider_runs.map((run) => (
                        <span key={`${run.provider}-${run.role}`}>{run.role} · {run.model} · {run.latency_ms}ms · {run.prompt_version}</span>
                      ))}
                    </div>
                  </div>
                )}
        ...
        <section className="section-block" id="revisao">
          <div className="section-heading">
            <div>
              ...
              <p>O ATLAS mostra por que uma análise precisa de uma pessoa antes de seguir.</p>
            </div>
          </div>
          ...
          <div className={`review-panel panel ${assessment.human_review_required ? "review-required" : "review-clear"}`}>
            <div>
              ...
              <p>
                O resultado é experimental. Scores representam features normalizadas e não são probabilidade calibrada,
                score de crédito ou rating regulatório.
              </p>
            </div>
        ...
        <footer className="footer">
          ...
          <span>Evidência antes da narrativa · Decisão humana por design</span>
        </footer>
      </div>
    </main>
  );
