...
function translateDriver(driver: string) {
  return driverTranslations[driver] ?? driver;
}

function translateReviewReason(reason: string) {
  return reviewReasonTranslations[reason] ?? reason;
}

function RiskBar({ score, band }: { score: number; band: RiskBand }) {
  return (
    <div className="risk-bar" aria-label={`${toPercent(score)}%`}>
      <span className={`risk-bar-fill risk-${band.toLowerCase()}`} style={{ width: `${toPercent(score)}%` }} />
    </div>
  );
}
...
function RiskFactorRow({ label, score, band, drivers }: { label: string; score: number; band: RiskBand; drivers: string[] }) {
  return (
    <div className="risk-factor-row">
      ...
      <div className="risk-factor-score"><span>{toPercent(score)}</span><small className={`risk-text risk-${band.toLowerCase()}`}>{bandLabel[band]}</small></div>
...
export default function HomePage() {
  ...
  async function runDualModelReview() {
    ...
    const evidence: EvidenceInput[] = [{
      ...
      payload: { sector: input.sector, region: input.region, note: "Synthetic user-controlled demo signals. Not an external factual source." },
      is_synthetic: true,
    }];
    try {
      ...
      setAiReview(null);
      setAiError(err instanceof Error ? err.message : "O contraditório independente não pôde ser executado.");
    } finally {
      setAiLoading(false);
    }
  }
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <div className="topbar-brand"><strong>ATLAS</strong><span>Inteligência de Risco SAC</span></div>
        <div className="topbar-actions"><EngineStatus mode={mode} /><span className="topbar-method">Evidência → risco → decisão humana</span></div>
      </header>
      ...
      <div className="content">
        <section className="search-section" id="resumo">
          <div className="section-intro">
            ...
            <p>O ATLAS separa exposição, sinais observados e lacunas de evidência para tornar a análise socioambiental e climática rastreável, contestável e revisável por pessoas.</p>
          </div>
          <form className="counterparty-form" onSubmit={submit}>
            <div className="search-row">
              <input aria-label="Empresa ou contraparte" className="company-search" value={input.company_name} onChange={(event) => updateText("company_name", event.target.value)} />
              <button className="button button-primary" type="submit" disabled={loading}>{loading ? "Analisando…" : "Analisar"}</button>
            </div>
            <div className="entity-fields">
              <label><span>Setor</span><input value={input.sector} onChange={(event) => updateText("sector", event.target.value)} /></label>
              <label><span>Região</span><input value={input.region} onChange={(event) => updateText("region", event.target.value)} /></label>
              <button className="button button-ghost" type="button" onClick={resetDemo}>Restaurar demonstração</button>
            </div>
            <details className="parameter-drawer">
              ...
              <div className="parameter-grid">
                {sliderFields.map((field) => {
                  ...
                  return <label className="parameter-field" key={field.key}><div><span>{field.label}</span><strong>{toPercent(value)}</strong></div><input aria-label={field.label} type="range" min="0" max="1" step="0.01" value={value} onChange={(event) => updateNumber(field.key, event.target.value)} /><small>{field.hint}</small></label>;
                })}
              </div>
            </details>
          </form>
          {error ? <div className="inline-alert inline-alert-error"><strong>Motor de risco indisponível.</strong><span>{error}. A prévia sintética continua visível.</span></div> : null}
        </section>

        <section className="entity-header">
          <div><h2>{assessment.company_name}</h2><p>{input.region} · {input.sector}</p></div>
          <div className="entity-badges"><span>Perfil sintético</span><span>Metodologia experimental</span></div>
        </section>
        ...
        <section className="risk-overview panel">
          <div className="risk-score-block"><span className="panel-label">Risco SAC</span><div className="primary-score"><strong>{toPercent(assessment.overall_score)}</strong><span className={`risk-text risk-${assessment.overall_band.toLowerCase()}`}>{bandLabel[assessment.overall_band]}</span></div><RiskBar score={assessment.overall_score} band={assessment.overall_band} /></div>
          <div className="decision-block">
            <span className="panel-label">Decisão</span>
            <strong>{assessment.human_review_required ? "Revisão humana necessária" : "Sem revisão obrigatória"}</strong>
            <p>{assessment.human_review_required ? "Sinais materiais ou incerteza relevante impedem o encerramento automático da análise." : "Nenhum gatilho obrigatório de revisão foi acionado nesta execução experimental."}</p>
            <a className="button button-review" href="#revisao">{assessment.human_review_required ? "Entender por quê" : "Ver decisão"}</a>
          </div>
          <div className="dimension-summary">
            ...
            <div><span>Confiança</span><strong>{toPercent(assessment.confidence)}%</strong></div>
            <div><span>Risco observado</span><strong>{toPercent(assessment.observed_risk.score)}</strong></div>
            <div><span>Sinal do modelo</span><strong>{mlPrediction ? `${toPercent(mlPrediction.predicted_material_risk_probability)}%` : "—"}</strong></div>
          </div>
        </section>

        <section className="section-block" id="evidencias">
          <div className="section-heading"><div><h2>Evidências</h2><p>O que sustenta a análise, o que é apenas contexto e onde ainda existe incerteza.</p></div><span className="section-note">Sinais sintéticos até a integração de dados públicos entrar nesta branch</span></div>
          <div className="evidence-legend" aria-label="Legenda de evidências"><span className="legend-item legend-context">Contexto</span><span className="legend-item legend-observed">Observado</span><span className="legend-item legend-unknown">Desconhecido</span></div>
          <div className="table-panel panel"><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Fonte</th><th>Sinal</th><th>Valor</th><th>Confiança</th><th>Estado</th></tr></thead><tbody>{evidenceRows.map((row) => <tr key={`${row.source}-${row.signal}`}><td>{row.source}</td><td>{row.signal}</td><td>{row.value}</td><td>{row.confidence}</td><td><span className={`table-status status-${row.tone}`}>{row.status}</span></td></tr>)}</tbody></table></div></div>
        </section>

        <section className="section-block" id="fatores">
          <div className="section-heading"><div><h2>Fatores de risco</h2><p>As dimensões permanecem separadas para evitar que um único número esconda a origem do risco.</p></div><span className="section-note">Escala heurística experimental de 0 a 100</span></div>
          <div className="risk-factor-panel panel">{dimensions.map(({ key, label, dimension }) => <RiskFactorRow key={key} label={label} score={dimension.score} band={dimension.band} drivers={dimension.drivers} />)}</div>
        </section>

        <section className="section-block" id="metodologia">
          <div className="section-heading"><div><h2>Metodologia</h2><p>A complexidade fica disponível para auditoria, mas não compete com a decisão principal.</p></div></div>
            ...
            <div className="methodology-grid">
              <article className="panel method-panel">
                <div className="method-steps">
                  <div><span>01</span><strong>Regras determinísticas</strong><p>Calculam o score e os gatilhos de revisão.</p></div>
                  <div><span>02</span><strong>Modelo estatístico</strong><p>Adiciona um segundo sinal treinado em dados sintéticos.</p></div>
                  <div><span>03</span><strong>Contraditório independente</strong><p>Uma análise propõe interpretação e outra procura afirmações sem suporte.</p></div>
                  <div><span>04</span><strong>Decisão humana</strong><p>Nenhuma camada de IA pode remover um bloqueio determinístico.</p></div>
                </div>
                <div className="model-metrics"><MethodMetric label="ROC-AUC" value={mlEvaluation ? mlEvaluation.roc_auc.toFixed(3) : "—"} /><MethodMetric label="Precisão" value={mlEvaluation ? mlEvaluation.precision.toFixed(3) : "—"} /><MethodMetric label="Recall" value={mlEvaluation ? mlEvaluation.recall.toFixed(3) : "—"} /><MethodMetric label="Brier score" value={mlEvaluation ? mlEvaluation.brier_score.toFixed(3) : "—"} /></div>
                <div className="method-footnote"><span>{mlPrediction?.model_version ?? "synthetic-logreg-v1"}</span><span>{mlPrediction?.dataset_version ?? "atlas-sac-synthetic-v1"}</span><span>Completude de evidência excluída das features de ML</span></div>
              </article>
              <article className="panel review-method-panel">
                <div className="review-method-head"><div><span className="panel-label">Contraditório independente</span><h3>Interpretar, contestar, escalar</h3></div><button className="button button-secondary" type="button" onClick={runDualModelReview} disabled={aiLoading}>{aiLoading ? "Revisando…" : "Executar contraditório"}</button></div>
                {aiError ? <div className="inline-alert inline-alert-error">{aiError}</div> : null}
                {!aiReview ? <div className="review-empty"><p>A camada de linguagem fica depois de regras e modelo estatístico. Ela pode explicar, contestar e pedir mais informação, mas não pode reescrever os scores determinísticos.</p></div> : <div className="review-result"><div className="review-row"><span>Análise</span><strong>{aiReview.analyst?.summary ?? "Indisponível"}</strong></div><div className="review-row"><span>Revisor</span><strong>{aiReview.reviewer?.verdict ?? "SEM SAÍDA"}</strong><p>{aiReview.reviewer?.rationale ?? aiReview.degradation_reason}</p></div><div className="review-row review-gate-row"><span>Portão de decisão</span><strong>{aiReview.decision_gate.replaceAll("_", " ")}</strong><p>{aiReview.disagreement ? "Houve discordância entre modelos. A revisão humana permanece obrigatória." : "Nenhuma discordância material foi registrada nesta execução."}</p></div><div className="provider-traces">{aiReview.provider_runs.map((run) => <span key={`${run.provider}-${run.role}`}>{run.role} · {run.model} · {run.latency_ms}ms · {run.prompt_version}</span>)}</div></div>}
              </article>
            </div>
          </details>
        </section>
        ...
        <section className="section-block" id="revisao">
          <div className="section-heading"><div><h2>Revisão humana</h2><p>O ATLAS mostra por que uma análise precisa de uma pessoa antes de seguir.</p></div></div>
          <div className={`review-panel panel ${assessment.human_review_required ? "review-required" : "review-clear"}`}>
            <div><span className="panel-label">Estado da decisão</span><h3>{assessment.human_review_required ? "Revisão humana necessária" : "Sem revisão obrigatória"}</h3><p>O resultado é experimental. Scores representam features normalizadas e não são probabilidade calibrada, score de crédito ou rating regulatório.</p></div>
            <div className="review-reasons"><span className="panel-label">Gatilhos</span><ul>{assessment.review_reasons.length ? assessment.review_reasons.map((reason) => <li key={reason}>{translateReviewReason(reason)}</li>) : <li>Nenhum gatilho obrigatório nesta execução.</li>}</ul></div>
          </div>
        </section>
