  ...
  return (
    <main className="app-shell">
      ...
      <aside className="sidebar" aria-label="Navegação principal">
        <div className="sidebar-group"><span className="sidebar-label">Análise</span><a className="sidebar-link active" href="#resumo">Resumo</a><a className="sidebar-link" href="#evidencias">Evidências</a><a className="sidebar-link" href="#fatores">Fatores de risco</a><a className="sidebar-link" href="#metodologia">Metodologia</a><a className="sidebar-link" href="#revisao">Revisão humana</a></div>
        <div className="sidebar-group sidebar-system"><span className="sidebar-label">Camadas</span><span className="system-item">Regras determinísticas</span><span className="system-item">Modelo estatístico</span><span className="system-item">Contraditório independente</span><span className="system-endpoint">{apiUrl()}</span></div>
      </aside>

      <div className="content">
        <section className="search-section" id="resumo">
          <div className="section-intro"><span>Avaliação de contraparte</span><h1>Entenda o risco antes da decisão.</h1><p>O ATLAS separa exposição, sinais observados e lacunas de evidência para tornar a análise socioambiental e climática rastreável, contestável e revisável por pessoas.</p></div>
          <form className="counterparty-form" onSubmit={submit}>
            <div className="search-row"><input aria-label="Empresa ou contraparte" className="company-search" value={input.company_name} onChange={(event) => updateText("company_name", event.target.value)} /><button className="button button-primary" type="submit" disabled={loading}>{loading ? "Analisando…" : "Analisar"}</button></div>
            <div className="entity-fields"><label><span>Setor</span><input value={input.sector} onChange={(event) => updateText("sector", event.target.value)} /></label><label><span>Região</span><input value={input.region} onChange={(event) => updateText("region", event.target.value)} /></label><button className="button button-ghost" type="button" onClick={resetDemo}>Restaurar demonstração</button></div>
            <details className="parameter-drawer"><summary>Parâmetros da demonstração</summary><div className="parameter-grid">{sliderFields.map((field) => { const value = input[field.key] as number; return <label className="parameter-field" key={field.key}><div><span>{field.label}</span><strong>{toPercent(value)}</strong></div><input aria-label={field.label} type="range" min="0" max="1" step="0.01" value={value} onChange={(event) => updateNumber(field.key, event.target.value)} /><small>{field.hint}</small></label>; })}</div></details>
          </form>
        ...
        <section className="risk-overview panel">
          ...
          <div className="decision-block"><span className="panel-label">Decisão</span><strong>{assessment.human_review_required ? "Revisão humana necessária" : "Sem revisão obrigatória"}</strong><p>{assessment.human_review_required ? "Sinais materiais ou incerteza relevante impedem o encerramento automático da análise." : "Nenhum gatilho obrigatório de revisão foi acionado nesta execução experimental."}</p><a className="button button-review" href="#revisao">{assessment.human_review_required ? "Entender por quê" : "Ver decisão"}</a></div>
